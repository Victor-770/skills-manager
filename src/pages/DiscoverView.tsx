...
import { CopySkillsDialog } from "@/components/discover/CopySkillsDialog";
import { AddDiscoveredSkillToCollectionDialog } from "@/components/discover/AddDiscoveredSkillToCollectionDialog";
...
export function DiscoverView() {
  ...
  const deleteDiscoveredSkillPermanently = useDiscoverStore(
  ...
  const [addToCollectionSkills, setAddToCollectionSkills] = useState<DiscoveredSkill[]>([]);
  ...
  const selectedDiscoveredSkills = useMemo(() => {
    const allSkills = discoveredProjects.flatMap((p) => p.skills);
    return allSkills.filter((s) => selectedSkillIds.has(s.id));
  }, [discoveredProjects, selectedSkillIds]);

  const handleAddToCollection = useCallback((skill: DiscoveredSkill) => {
  ...
  const handleBatchAddToCollection = useCallback(() => {
    ...
    setAddToCollectionSkills(selectedDiscoveredSkills);
    setIsAddToCollectionOpen(true);
  ...
  const handlePermanentDelete = useCallback(
    async (skillId: string) => {
      ...
      try {
        const result = await deleteDiscoveredSkillPermanently(skillId);
        await Promise.all([refreshCounts(), refreshDiscoverCounts()]);
        toast.success(
          t("discover.permanentDeleteSuccess", {
            path: result.removed_dir,
          })
        );
      } catch (err) {
        toast.error(t("discover.permanentDeleteError", { error: String(err) }));
      } finally {
        setDeletingIds((prev) => {
          const next = new Set(prev);
          next.delete(skillId);
    ...
    [deleteDiscoveredSkillPermanently, refreshCounts, refreshDiscoverCounts, t]
  );
    ...
    <div className="flex flex-col h-full">
            ...
            <>
              ...
              <div className="px-6 py-3 border-b border-border flex items-center gap-3">
                ...
                <Button
                  ...
                  disabled={selectedProject.skills.length === 0}
                  title={t("discover.copyToProject")}
                >
                  <CopyPlus className="size-3.5 mr-1" />
                  ...
                  <VirtualizedList
                    ...
                    renderItem={(skill) => (
                      <UnifiedSkillCard
                        ...
                        onAddToCollection={() => handleAddToCollection(skill)}
                        onPermanentDelete={() => handlePermanentDelete(skill.id)}
                        isLoading={importingIds.has(skill.id) || deletingIds.has(skill.id)}
                        className="h-[120px]"
                      />
                    )}
                  />
        ...
        <div className="border-t border-border px-6 py-3 flex items-center gap-3 bg-muted/20">
          ...
          <div className="flex items-center gap-2 ml-auto">
            ...
            >
              ...
              {t("discover.addSelectedToCollection", { defaultValue: "加入技能集" })}
            </Button>
            <Button
              variant="outline"
              size="sm"
            ...
            <Button
              ...
              onClick={() => {
                if (!window.confirm(t("discover.permanentDeleteSelectedConfirm", { count: selectedSkillIds.size }))) return;
            ...
            >
              ...
              {t("discover.permanentDeleteSelected")}
            </Button>
            <Button
              variant="ghost"
              size="sm"
      ...
      {selectedProject && (
        <CopySkillsDialog
          ...
          onComplete={refreshDiscoverCounts}
        />
      )}

      {/* Add project skill(s) into a collection */}
      {isAddToCollectionOpen && (
        <AddDiscoveredSkillToCollectionDialog
          open={isAddToCollectionOpen}
          onOpenChange={(open) => {
            setIsAddToCollectionOpen(open);
            if (!open) setAddToCollectionSkills([]);
          }}
          skills={addToCollectionSkills}
          onComplete={() => {
            if (addToCollectionSkills.length > 1) {
              clearSelection();
            }
