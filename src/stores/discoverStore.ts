...
interface DiscoverState {
  ...
  ) => Promise<CopyProjectSkillsPlan>;
  executeCopyProjectSkills: (
    sourceProjectPath: string,
    targetProjectPath: string,
    overwrite: boolean
  ) => Promise<CopyProjectSkillsResult>;
  // Collection ↔ project integration
  getProjectPlatformDirs: (projectPath: string) => Promise<string[]>;
  previewInstallCollectionToProject: (
    collectionId: string,
    projectPath: string,
    platformDir: string
  ) => Promise<CollectionInstallPlan>;
  ...
  previewAddDiscoveredSkillToCollection: (
    collectionId: string,
    discoveredSkillId: string
  ) => Promise<AddDiscoveredSkillPlan>;
  executeAddDiscoveredSkillToCollection: (
    collectionId: string,
    discoveredSkillId: string,
    overwrite: boolean
  ) => Promise<AddDiscoveredSkillResult>;
  batchAddDiscoveredSkillsToCollections: (
    collectionIds: string[],
    discoveredSkillIds: string[]
  ) => Promise<void>;
  deleteDiscoveredSkillPermanently: (
    discoveredSkillId: string
  ) => Promise<DeleteDiscoveredSkillResult>;
...
export const useDiscoverStore = create<DiscoverState>((set, get) => ({
  ...
  previewCopyProjectSkills: async (sourceProjectPath, targetProjectPath) => {
    set({ error: null });
    try {
      return await invoke<CopyProjectSkillsPlan>("preview_copy_project_skills", {
        sourceProjectPath,
        targetProjectPath,
      });
    } catch (err) {
  ...
  executeCopyProjectSkills: async (
    sourceProjectPath,
    targetProjectPath,
    overwrite
  ) => {
    set({ error: null });
    try {
      return await invoke<CopyProjectSkillsResult>("execute_copy_project_skills", {
        sourceProjectPath,
        targetProjectPath,
        overwrite,
      });
  ...
  getProjectPlatformDirs: async (projectPath) => {
    set({ error: null });
    try {
      return await invoke<string[]>("get_project_platform_dirs", { projectPath });
    } catch (err) {
      set({ error: String(err) });
      throw err;
    }
  ...
  previewInstallCollectionToProject: async (collectionId, projectPath, platformDir) => {
    set({ error: null });
    try {
      return await invoke<CollectionInstallPlan>(
        "preview_install_collection_to_project",
        { collectionId, projectPath, platformDir }
      );
    } catch (err) {
      set({ error: String(err) });
      throw err;
  ...
  executeInstallCollectionToProject: async (
    collectionId,
    projectPath,
    platformDir,
    overwrite
    ...
    set({ error: null });
    try {
      return await invoke<CollectionInstallResult>(
        "execute_install_collection_to_project",
        { collectionId, projectPath, platformDir, overwrite }
      );
    } catch (err) {
      set({ error: String(err) });
      throw err;
  ...
  previewAddDiscoveredSkillToCollection: async (collectionId, discoveredSkillId) => {
    set({ error: null });
    try {
      return await invoke<AddDiscoveredSkillPlan>(
        "preview_add_discovered_skill_to_collection",
        { collectionId, discoveredSkillId }
      );
    } catch (err) {
      set({ error: String(err) });
      throw err;
  ...
  executeAddDiscoveredSkillToCollection: async (
    collectionId,
    discoveredSkillId,
    overwrite
  ) => {
    set({ error: null });
    try {
      return await invoke<AddDiscoveredSkillResult>(
        "execute_add_discovered_skill_to_collection",
        { collectionId, discoveredSkillId, overwrite }
      );
    } catch (err) {
      set({ error: String(err) });
      throw err;
  ...
  batchAddDiscoveredSkillsToCollections: async (
    collectionIds,
    discoveredSkillIds
  ) => {
    set({ error: null });
    try {
      await invoke("batch_add_discovered_skills_to_collections", {
        collectionIds,
        discoveredSkillIds,
      });
    } catch (err) {
  ...
  deleteDiscoveredSkillPermanently: async (discoveredSkillId) => {
    set({ error: null });
    try {
      const result = await invoke<DeleteDiscoveredSkillResult>(
        "delete_discovered_skill_permanently",
        { discoveredSkillId }
      );
      // Remove the skill from local results so the UI reflects the deletion.
      set((state) => {
        const updatedProjects = state.discoveredProjects
          .map((p) => ({
            ...p,
            skills: p.skills.filter((s) => s.id !== discoveredSkillId),
          }))
          .filter((p) => p.skills.length > 0);
        const totalSkills = updatedProjects.reduce(
          (sum, p) => sum + p.skills.length,
          0
        );
        const newSelection = new Set(state.selectedSkillIds);
        newSelection.delete(discoveredSkillId);
        return {
          discoveredProjects: updatedProjects,
          totalSkillsFound: totalSkills,
          selectedSkillIds: newSelection,
