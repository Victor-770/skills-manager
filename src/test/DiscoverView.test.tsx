  ...
  useTranslation: () => ({
    t: (key: string, params?: Record<string, unknown>) => {
      const map: Record<string, string> = {
        ...
        "discover.copyToProject": "Copy to project",
        "discover.copyDesktopOnly": "Desktop only",
        "discover.addCollection": "Add to Collection",
        "discover.permanentDelete": "Permanently delete (Shift+Delete)",
        "discover.permanentDeleteSuccess": "Permanently deleted",
        "discover.permanentDeleteError": "Failed to delete",
...
function buildDiscoverStoreState(overrides = {}) {
  return {
    ...
    previewCopyProjectSkills: vi.fn(),
    executeCopyProjectSkills: vi.fn(),
    getProjectPlatformDirs: vi.fn(),
    previewInstallCollectionToProject: vi.fn(),
    executeInstallCollectionToProject: vi.fn(),
    previewAddDiscoveredSkillToCollection: vi.fn(),
    executeAddDiscoveredSkillToCollection: vi.fn(),
...
describe("DiscoverView", () => {
  ...
  it("shows a copy-to-project action when a project is selected", () => {
    const encoded = encodeURIComponent("/home/user/projects/my-app");
    renderDiscoverView(`/discover/${encoded}`);
    expect(
  ...
  it("shows add-to-collection actions on discovered skill cards", () => {
    const encoded = encodeURIComponent("/home/user/projects/my-app");
    renderDiscoverView(`/discover/${encoded}`);
    // Both fixture skills should expose the Add to Collection action.
    expect(
      screen.getAllByRole("button", { name: "Add to Collection" }).length
    ).toBeGreaterThanOrEqual(2);
  });

  it("opens the add-to-collection dialog when the card action is clicked", () => {
    const encoded = encodeURIComponent("/home/user/projects/my-app");
    renderDiscoverView(`/discover/${encoded}`);
    fireEvent.click(
  ...
  it("shows permanent-delete actions and triggers backend deletion", async () => {
    const encoded = encodeURIComponent("/home/user/projects/my-app");
    const state = buildDiscoverStoreState({
      deleteDiscoveredSkillPermanently: vi.fn().mockResolvedValue({
    ...
    const buttons = screen.getAllByRole("button", {
      name: "Permanently delete (Shift+Delete)",
    });
    expect(buttons.length).toBeGreaterThanOrEqual(2);
    fireEvent.click(buttons[0]);
    // Inline confirm arm step.
    fireEvent.click(screen.getAllByRole("button", { name: "Confirm delete" })[0]);
    await waitFor(() => {
      expect(state.deleteDiscoveredSkillPermanently).toHaveBeenCalled();
    });
  });
