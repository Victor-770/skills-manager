...
// ─── Project-to-project skill copy ─────────────────────────────────────────────

/// A single skill considered for a project-to-project copy, with its relative
/// location inside the source project and whether the target location is taken.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct CopySkillItem {
    /// Skill directory name (last path component of the skill dir).
    pub dir_name: String,
    pub skill_name: String,
    /// Directory of the source skill relative to the source project root.
    /// The copy always lands at `target_project_path/<relative_path>`.
    pub relative_path: String,
    /// Qualified id of the source discovered skill (used to re-register the
    /// copied skill in the Discover cache for the target project).
    pub source_skill_id: String,
    pub source_dir: String,
    pub target_dir: String,
    pub platform_id: String,
    pub platform_name: String,
    /// False when the source directory no longer exists on disk.
    pub source_exists: bool,
    /// True when `target_dir` already exists (file, dir, or symlink).
    pub conflict: bool,
}

/// Preview of copying all discovered skills of one project into another.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct CopyPlan {
    ...
    pub skills: Vec<CopySkillItem>,
}

/// Per-skill outcome of an executed project-to-project copy.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct CopySkillOutcome {
    pub dir_name: String,
    pub skill_name: String,
    pub relative_path: String,
    pub copied: bool,
...
/// Aggregate result of executing a project-to-project copy.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct CopyResult {
    pub copied: usize,
    pub skipped: usize,
    pub overwritten: usize,
    pub skills: Vec<CopySkillOutcome>,
}

/// Whether a path exists as file, dir, or (possibly broken) symlink.
...
/// Basic validation for a project-to-project copy: projects must differ and
/// exist on disk, and neither may contain the other.
fn validate_copy_projects(source: &Path, target: &Path) -> Result<(), String> {
    if same_or_nested(source, target) {
        return Err(
            "Source and target must be different projects (they cannot contain one another)"
                .to_string(),
        );
    }
    if !source.is_dir() {
...
/// Build a copy plan from the Discover cache: every discovered skill whose
/// `project_path` equals the source project is mapped to the equivalent
/// relative location inside the target project.
async fn build_copy_plan(
    pool: &DbPool,
    source_project_path: &str,
    target_project_path: &str,
) -> Result<CopyPlan, String> {
    let source = Path::new(source_project_path);
    let target = Path::new(target_project_path);
    validate_copy_projects(source, target)?;

    let rows = db::get_all_discovered_skills(pool).await?;
    let source_name = rows
        .iter()
        .find(|row| row.project_path == source_project_path)
        .map(|row| row.project_name.clone())
        .unwrap_or_else(|| file_name_or_unknown(source));
    let target_name = rows
        .iter()
        .find(|row| row.project_path == target_project_path)
        .map(|row| row.project_name.clone())
        .unwrap_or_else(|| file_name_or_unknown(target));

    let mut skills: Vec<CopySkillItem> = Vec::new();
    for row in rows {
        if row.project_path != source_project_path {
            continue;
        }
        let dir_path = Path::new(&row.dir_path);
        // Only skills physically located under the source project root can be
        // mapped to a target location.
        let Ok(relative) = dir_path.strip_prefix(source) else {
            continue;
        };
        ...
        skills.push(CopySkillItem {
            dir_name: selected_skill_dir_name(&row.dir_path),
            skill_name: row.name.clone(),
            relative_path: path_to_string(&relative_path),
            source_skill_id: row.id.clone(),
            source_dir: row.dir_path.clone(),
            target_dir: path_to_string(&target_dir),
            platform_id: row.platform_id.clone(),
            platform_name: platform_display_name(&row.platform_id),
            source_exists: dir_path.is_dir(),
            conflict: path_exists_on_disk(&target_dir),
        });
    }
    skills.sort_by(|a, b| {
        a.relative_path
    ...
    let conflict_count = skills.iter().filter(|item| item.conflict).count();
    let missing_count = skills.iter().filter(|item| !item.source_exists).count();
...
/// Remove whatever occupies `path` (file, directory, or symlink) so a copy can
/// overwrite it. Missing paths are a no-op.
...
/// Execute a copy plan. With `overwrite` true, conflicting target locations are
/// replaced; otherwise they are skipped. Per-skill failures are recorded in the
/// outcome instead of aborting the whole batch.
fn execute_copy_plan_impl(plan: &CopyPlan, overwrite: bool) -> Result<CopyResult, String> {
    let mut copied = 0;
    let mut skipped = 0;
    let mut overwritten = 0;
    let mut outcomes: Vec<CopySkillOutcome> = Vec::with_capacity(plan.skills.len());

    for item in &plan.skills {
        let src = Path::new(&item.source_dir);
        let dst = Path::new(&item.target_dir);
        let mut outcome = CopySkillOutcome {
        ...
        if !item.source_exists {
            ...
            outcome.reason = Some("Source skill directory no longer exists".to_string());
            skipped += 1;
        } else if item.conflict && !overwrite {
            outcome.skipped = true;
            outcome.reason = Some(
                "A skill at the same location already exists in the target project".to_string(),
            );
            skipped += 1;
        } else {
            if item.conflict {
                if let Err(err) = remove_existing_path(dst) {
                    outcome.skipped = true;
                    outcome.reason = Some(err);
                    skipped += 1;
            ...
            match super::linker::copy_dir_all(src, dst) {
                Ok(()) => {
                    outcome.copied = true;
                    copied += 1;
                }
...
/// Re-register every successfully copied skill inside the Discover cache as a
/// row belonging to the target project, so the target shows the new skills
/// immediately without a full rescan. Rows are best-effort upserts; the next
/// rescan reconciles them with the real filesystem contents.
async fn record_copied_skills_in_discover_cache(
    pool: &DbPool,
    plan: &CopyPlan,
    result: &CopyResult,
) -> Result<(), String> {
    let now = Utc::now().to_rfc3339();
    let target_kebab = plan
        .target_project_name
        .to_lowercase()
        .replace(' ', "-");
    ...
    for outcome in &result.skills {
        ...
        let Some(item) = plan
            .skills
            .iter()
            .find(|item| item.relative_path == outcome.relative_path)
        else {
            continue;
        };

        let dir_name = &item.dir_name;
        let new_id = format!("{}__{}__{}", item.platform_id, target_kebab, dir_name);
        let target_dir = Path::new(&item.target_dir);
        let file_path = path_to_string(&target_dir.join("SKILL.md"));

        db::insert_discovered_skill(
            pool,
            &new_id,
            &item.skill_name,
            None, // description is not carried by the cache row; rescan repopulates it
            &file_path,
            &item.target_dir,
            &plan.target_project_path,
...
/// Preview which skills of the source project would be copied into the target
/// project and which target locations are already taken (conflicts).
#[tauri::command]
pub async fn preview_copy_project_skills(
    state: State<'_, AppState>,
    source_project_path: String,
    target_project_path: String,
) -> Result<CopyPlan, String> {
    build_copy_plan(&state.db, &source_project_path, &target_project_path).await
}

/// Copy all discovered skills of the source project into the target project.
/// Conflicting target locations are overwritten when `overwrite` is true and
/// skipped otherwise.
#[tauri::command]
pub async fn execute_copy_project_skills(
    state: State<'_, AppState>,
    source_project_path: String,
    target_project_path: String,
    overwrite: bool,
) -> Result<CopyResult, String> {
    let plan = build_copy_plan(&state.db, &source_project_path, &target_project_path).await?;
    let result = execute_copy_plan_impl(&plan, overwrite)?;
    record_copied_skills_in_discover_cache(&state.db, &plan, &result).await?;
    Ok(result)
}

// ─── Collection ↔ Project integration ──────────────────────────────────────────

/// One skill of a collection mapped to its target location inside a project.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct CollectionInstallItem {
    pub dir_name: String,
    pub skill_name: String,
    /// Display path relative to the project root (e.g. `.claude/skills/foo`).
    pub relative_path: String,
    pub source_skill_id: String,
    pub source_dir: String,
    pub target_dir: String,
...
/// Preview of installing every skill of a collection into a project folder.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct CollectionInstallPlan {
    ...
    pub conflict_count: usize,
    pub missing_count: usize,
    pub skills: Vec<CollectionInstallItem>,
}

/// Per-skill outcome of a collection→project install.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct CollectionInstallOutcome {
    pub dir_name: String,
    pub skill_name: String,
    pub relative_path: String,
    pub copied: bool,
...
/// Aggregate result of executing a collection→project install.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct CollectionInstallResult {
    pub copied: usize,
    pub skipped: usize,
    pub overwritten: usize,
    pub skills: Vec<CollectionInstallOutcome>,
}

/// Preview of adding one discovered (project) skill into a collection.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct AddDiscoveredSkillPlan {
    pub collection_id: String,
    pub collection_name: String,
    pub discovered_skill_id: String,
    pub skill_dir_name: String,
    pub skill_name: String,
    pub central_target: String,
    /// A central skill with the same directory name already exists.
    pub central_conflict: bool,
    /// The collection already contains a skill with this directory name.
    pub already_in_collection: bool,
}

/// Result of executing an add-discovered-skill-to-collection.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct AddDiscoveredSkillResult {
    pub skill_id: String,
    pub added_to_collection: bool,
    pub overwrote_central: bool,
}

/// Result of permanently deleting a discovered (project) skill.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct DeleteDiscoveredSkillResult {
    pub removed_dir: String,
    pub row_deleted: bool,
}

/// Resolve the on-disk source directory of a collection skill. Central skills
/// live at `canonical_path`; fall back to the parent of `file_path`.
fn collection_skill_source_dir(skill: &db::Skill) -> Option<PathBuf> {
    if let Some(canonical) = &skill.canonical_path {
        let dir = PathBuf::from(canonical);
        if dir.join("SKILL.md").exists() || dir.is_dir() {
            return Some(dir);
        }
    }
    Path::new(&skill.file_path).parent().map(Path::to_path_buf)
}

/// Map a project-relative platform dir (e.g. `.claude/skills`) back to the
/// built-in agent that owns it, falling back to a derived generic id.
fn platform_for_rel_dir(pool: &DbPool, rel_dir: &str) -> (String, String) {
    for (agent_id, display_name, rel) in platform_skill_patterns(pool) {
        if path_to_string(&rel) == rel_dir {
            return (agent_id, display_name);
        }
    }
    let leaf = Path::new(rel_dir)
        .parent()
        ...
        .trim_start_matches('.')
        .to_string();
    (format!("project-{}", leaf), rel_dir.to_string())
}

/// Distinct project-relative platform skill dirs detected for a project.
pub async fn project_platform_dirs_impl(
    pool: &DbPool,
    project_path: &str,
) -> Result<Vec<String>, String> {
    let rows = db::get_all_discovered_skills(pool).await?;
    let mut dirs: BTreeSet<String> = BTreeSet::new();
    for row in rows.iter().filter(|r| r.project_path == project_path) {
        let dir_path = Path::new(&row.dir_path);
        if let Ok(rel) = dir_path.strip_prefix(Path::new(project_path)) {
            if let Some(parent) = rel.parent() {
                if !parent.as_os_str().is_empty() {
                    dirs.insert(path_to_string(parent));
                }
            }
        }
    }
...
/// Build a preview plan for installing a collection into a project folder.
async fn build_collection_install_plan(
    pool: &DbPool,
    collection_id: &str,
    project_path: &str,
    platform_dir: &str,
    ...
    if platform_dir.trim().is_empty() || platform_dir == "/" {
        return Err("A target platform directory is required (e.g. .claude/skills)".to_string());
    }
    ...
    let project_name = db::get_all_discovered_skills(pool)
        .await?
        .iter()
        .find(|row| row.project_path == project_path)
        .map(|row| row.project_name.clone())
        .unwrap_or_else(|| file_name_or_unknown(project));

    let (platform_id, platform_name) = platform_for_rel_dir(pool, platform_dir);

    let mut items: Vec<CollectionInstallItem> = Vec::new();
    for skill in &skills {
        let Some(source_dir) = collection_skill_source_dir(skill) else {
            continue;
        };
        let dir_name = selected_skill_dir_name(&source_dir.to_string_lossy());
        let dir_name = if dir_name == "unknown" {
            skill.id.clone()
        } else {
            dir_name
        };
        let target_dir = project.join(platform_dir).join(&dir_name);
        items.push(CollectionInstallItem {
            dir_name: dir_name.clone(),
            skill_name: skill.name.clone(),
            relative_path: format!("{}/{}", platform_dir, dir_name),
    ...
    let conflict_count = items.iter().filter(|item| item.conflict).count();
    let missing_count = items.iter().filter(|item| !item.source_exists).count();

    Ok(CollectionInstallPlan {
        collection_id: collection.id.clone(),
        collection_name: collection.name.clone(),
        target_project_path: project_path.to_string(),
...
/// Execute a collection→project install plan. With `overwrite` true, conflicting
/// target locations are replaced; otherwise they are skipped.
fn execute_collection_install_impl(
    ...
    let mut skipped = 0;
    let mut overwritten = 0;
    let mut outcomes: Vec<CollectionInstallOutcome> = Vec::with_capacity(plan.skills.len());

    for item in &plan.skills {
        let src = Path::new(&item.source_dir);
        let dst = Path::new(&item.target_dir);
        let mut outcome = CollectionInstallOutcome {
            dir_name: item.dir_name.clone(),
            skill_name: item.skill_name.clone(),
            relative_path: item.relative_path.clone(),
            copied: false,
...
/// Re-register skills installed from a collection into the Discover cache so the
/// target project shows them immediately without a full rescan.
async fn record_collection_install_in_discover_cache(
    pool: &DbPool,
    plan: &CollectionInstallPlan,
...
/// Preview adding one discovered (project) skill into a collection.
async fn build_add_discovered_skill_plan(
    pool: &DbPool,
    collection_id: &str,
    discovered_skill_id: &str,
    _central_dir: &Path,
) -> Result<AddDiscoveredSkillPlan, String> {
    let collection = db::get_collection_by_id(pool, collection_id)
        .await?
        .ok_or_else(|| format!("Collection '{}' not found", collection_id))?;
    let skill = db::get_discovered_skill_by_id(pool, discovered_skill_id)
    ...
    let skill_dir_name = Path::new(&skill.dir_path)
        ...
        .to_string();
    let collection_skills = db::get_collection_skills(pool, collection_id).await?;

    Ok(AddDiscoveredSkillPlan {
        collection_id: collection.id.clone(),
        collection_name: collection.name.clone(),
        discovered_skill_id: discovered_skill_id.to_string(),
        skill_dir_name: skill_dir_name.clone(),
        skill_name: skill.name.clone(),
        central_target: String::new(),
        central_conflict: false,
        already_in_collection: collection_skills
            .iter()
            .any(|s| s.id == skill_dir_name),
    })
}

/// Add one discovered (project) skill to a collection without copying it to central.
async fn execute_add_discovered_skill_to_collection_impl(
    pool: &DbPool,
    collection_id: &str,
    discovered_skill_id: &str,
    _overwrite: bool,
    central_dir: &Path,
) -> Result<AddDiscoveredSkillResult, String> {
    ...
    let skill_md_path = Path::new(&skill.file_path);
    let info = super::scanner::parse_skill_md(skill_md_path);
    let is_already_central = central_dir.join(&skill_dir_name).exists();
    let now = Utc::now().to_rfc3339();
    let db_skill = db::Skill {
    ...
    db::add_skill_to_collection(pool, collection_id, &skill_dir_name).await?;

    Ok(AddDiscoveredSkillResult {
        skill_id: skill_dir_name,
...
/// Batch add multiple discovered skills to multiple collections without copying to central.
pub async fn batch_add_discovered_skills_to_collections_impl(
    pool: &DbPool,
    collection_ids: &[String],
    discovered_skill_ids: &[String],
    central_dir: &Path,
) -> Result<(), String> {
    if collection_ids.is_empty() || discovered_skill_ids.is_empty() {
        return Ok(());
    }

    // Verify all collections exist
    for col_id in collection_ids {
        db::get_collection_by_id(pool, col_id)
            .await?
            .ok_or_else(|| format!("Collection '{}' not found", col_id))?;
    }

    for disc_id in discovered_skill_ids {
        let skill = match db::get_discovered_skill_by_id(pool, disc_id).await? {
            Some(s) => s,
            None => continue,
        };
        ...
        for col_id in collection_ids {
            db::add_skill_to_collection(pool, col_id, &skill_dir_name).await?;
        }
    }
...
/// Tauri command: distinct project-relative platform skill dirs of a project.
#[tauri::command]
pub async fn get_project_platform_dirs(
    state: State<'_, AppState>,
    project_path: String,
) -> Result<Vec<String>, String> {
    project_platform_dirs_impl(&state.db, &project_path).await
}

/// Tauri command: preview installing a collection into a project folder.
#[tauri::command]
pub async fn preview_install_collection_to_project(
    state: State<'_, AppState>,
    collection_id: String,
    project_path: String,
    platform_dir: String,
) -> Result<CollectionInstallPlan, String> {
    build_collection_install_plan(&state.db, &collection_id, &project_path, &platform_dir).await
}

/// Tauri command: install every skill of a collection into a project folder.
#[tauri::command]
pub async fn execute_install_collection_to_project(
    state: State<'_, AppState>,
    collection_id: String,
    project_path: String,
    platform_dir: String,
    ...
    let plan =
        build_collection_install_plan(&state.db, &collection_id, &project_path, &platform_dir)
            .await?;
    let result = execute_collection_install_impl(&plan, overwrite)?;
    record_collection_install_in_discover_cache(&state.db, &plan, &result).await?;
    Ok(result)
}

/// Tauri command: preview adding a discovered (project) skill into a collection.
#[tauri::command]
pub async fn preview_add_discovered_skill_to_collection(
    state: State<'_, AppState>,
    collection_id: String,
    discovered_skill_id: String,
) -> Result<AddDiscoveredSkillPlan, String> {
    build_add_discovered_skill_plan(
        &state.db,
        &collection_id,
        &discovered_skill_id,
        &central_skills_dir(),
...
/// Tauri command: add a discovered (project) skill into a collection without copying to central.
#[tauri::command]
pub async fn execute_add_discovered_skill_to_collection(
    state: State<'_, AppState>,
    collection_id: String,
    discovered_skill_id: String,
    overwrite: bool,
) -> Result<AddDiscoveredSkillResult, String> {
    execute_add_discovered_skill_to_collection_impl(
        &state.db,
        &collection_id,
        &discovered_skill_id,
        overwrite,
...
/// Tauri command: batch add discovered skills to collections.
#[tauri::command]
pub async fn batch_add_discovered_skills_to_collections(
    state: State<'_, AppState>,
    collection_ids: Vec<String>,
    discovered_skill_ids: Vec<String>,
) -> Result<(), String> {
    batch_add_discovered_skills_to_collections_impl(
        &state.db,
        &collection_ids,
...
/// Permanently delete a discovered (project) skill from disk (like Shift+Delete —
/// bypasses the recycle bin) and remove its cache row.
async fn delete_discovered_skill_permanently_impl(
    pool: &DbPool,
    discovered_skill_id: &str,
) -> Result<DeleteDiscoveredSkillResult, String> {
    let skill = db::get_discovered_skill_by_id(pool, discovered_skill_id)
        .await?
        .ok_or_else(|| format!("Discovered skill '{}' not found", discovered_skill_id))?;
        ...
        Ok(meta) => {
            if meta.file_type().is_symlink() {
                ...
                std::fs::remove_file(&dir)
                    .map_err(|e| format!("Failed to remove file '{}': {}", dir.display(), e))?;
            }
        }
        Err(_) => { /* already gone from disk — still drop the cache row */ }
    }

    db::delete_discovered_skill(pool, discovered_skill_id).await?;

    Ok(DeleteDiscoveredSkillResult {
        removed_dir: skill.dir_path.clone(),
        row_deleted: true,
    })
}

/// Tauri command: permanently delete a discovered (project) skill from disk
/// (like Shift+Delete — bypasses the recycle bin) and remove its cache row.
#[tauri::command]
pub async fn delete_discovered_skill_permanently(
    state: State<'_, AppState>,
    discovered_skill_id: String,
) -> Result<DeleteDiscoveredSkillResult, String> {
    delete_discovered_skill_permanently_impl(&state.db, &discovered_skill_id).await
}
...
mod tests {
    ...
    async fn seed_discovered_for_copy(
        pool: &DbPool,
        id: &str,
        name: &str,
        dir_path: &str,
    ...
    fn create_copy_source_skill(parent: &Path, rel: &str, marker: &str) -> PathBuf {
    ...
    #[tokio::test]
    async fn test_copy_plan_previews_skills_and_conflicts() {
        let tmp = tempfile::TempDir::new().unwrap();
        let pool = setup_test_db().await;

        let source = tmp.path().join("proj-a");
        let target = tmp.path().join("proj-b");
        std::fs::create_dir_all(&source).unwrap();
        std::fs::create_dir_all(&target).unwrap();

        create_copy_source_skill(&source, ".claude/skills/skill-a", "a");
        create_copy_source_skill(&source, ".claude/skills/skill-b", "b");
        create_copy_source_skill(&target, ".claude/skills/skill-b", "target-b");

        seed_discovered_for_copy(
            &pool,
            "claude-code__proj-a__skill-a",
            "skill-a",
            &source.join(".claude/skills/skill-a").to_string_lossy(),
        ...
        seed_discovered_for_copy(
            &pool,
            "claude-code__proj-a__skill-b",
            "skill-b",
            &source.join(".claude/skills/skill-b").to_string_lossy(),
            &source.to_string_lossy(),
            "proj-a",
            "claude-code",
        )
        ...
        let plan = build_copy_plan(&pool, &source.to_string_lossy(), &target.to_string_lossy())
            .await
            .unwrap();

        assert_eq!(plan.source_project_name, "proj-a");
        assert_eq!(plan.target_project_name, "proj-b");
        assert_eq!(plan.total_count, 2);
        assert_eq!(plan.conflict_count, 1);
        assert_eq!(plan.missing_count, 0);

        let skill_a = plan.skills.iter().find(|s| s.dir_name == "skill-a").unwrap();
        assert!(!skill_a.conflict);
        assert_eq!(skill_a.relative_path, ".claude/skills/skill-a");
        assert_eq!(
            skill_a.target_dir,
            target.join(".claude/skills/skill-a").to_string_lossy()
        );

        let skill_b = plan.skills.iter().find(|s| s.dir_name == "skill-b").unwrap();
        assert!(skill_b.conflict);
        assert_eq!(skill_b.source_skill_id, "claude-code__proj-a__skill-b");
    }

    #[tokio::test]
    async fn test_copy_plan_skips_rows_outside_source_root() {
        let tmp = tempfile::TempDir::new().unwrap();
        let pool = setup_test_db().await;
        ...
        std::fs::create_dir_all(&source).unwrap();
        std::fs::create_dir_all(&target).unwrap();
        create_copy_source_skill(&source, ".agents/skills/kept-skill", "kept");

        seed_discovered_for_copy(
            &pool,
            "codex__proj-a__kept-skill",
            "kept-skill",
            &source.join(".agents/skills/kept-skill").to_string_lossy(),
            &source.to_string_lossy(),
            "proj-a",
            "codex",
        )
        .await;
        // A stale row pointing outside the source root must not be copied.
        seed_discovered_for_copy(
            &pool,
            "claude-code__proj-a__stale",
            "stale",
            "/elsewhere/stale-skill",
            &source.to_string_lossy(),
            "proj-a",
            "claude-code",
        )
        ...
        assert_eq!(plan.skills[0].dir_name, "kept-skill");
    }

    #[tokio::test]
    async fn test_copy_plan_rejects_same_or_nested_projects() {
        let tmp = tempfile::TempDir::new().unwrap();
        let pool = setup_test_db().await;
        ...
        // Same project.
        let err = build_copy_plan(&pool, &source.to_string_lossy(), &source.to_string_lossy())
            .await
            .unwrap_err();
        assert!(err.contains("different"), "unexpected error: {}", err);

        // Nested project.
        let nested = tmp.path().join("proj-a/nested");
        std::fs::create_dir_all(&nested).unwrap();
        let err = build_copy_plan(&pool, &source.to_string_lossy(), &nested.to_string_lossy())
            .await
            .unwrap_err();
        assert!(err.contains("different"), "unexpected error: {}", err);
    }
    ...
    #[tokio::test]
    async fn test_execute_copy_skips_conflicts_without_overwrite() {
        let tmp = tempfile::TempDir::new().unwrap();
        let pool = setup_test_db().await;
        ...
        for (id, dir, name) in [
            ("claude-code__proj-a__skill-a", "skill-a", "skill-a"),
            ("claude-code__proj-a__skill-b", "skill-b", "skill-b"),
        ] {
            seed_discovered_for_copy(
                &pool,
                id,
                name,
                &source.join(format!(".claude/skills/{}", dir)).to_string_lossy(),
                &source.to_string_lossy(),
                "proj-a",
                "claude-code",
            )
        ...
        let plan = build_copy_plan(&pool, &source.to_string_lossy(), &target.to_string_lossy())
            .await
            .unwrap();
        let result = execute_copy_plan_impl(&plan, false).unwrap();
        ...
        // New skill copied.
        assert!(target.join(".claude/skills/skill-a/SKILL.md").exists());
        // Conflicting skill untouched.
        assert_eq!(
            std::fs::read_to_string(target.join(".claude/skills/skill-b/SKILL.md")).unwrap(),
            "---\nname: target-b\n---\n"
        );

        // Execution must be idempotent: re-preview (fresh conflict flags) and
        // re-run with overwrite=false skips everything already present.
        let fresh_plan =
            build_copy_plan(&pool, &source.to_string_lossy(), &target.to_string_lossy())
                .await
                .unwrap();
        assert_eq!(fresh_plan.conflict_count, 2);
        let result = execute_copy_plan_impl(&fresh_plan, false).unwrap();
    ...
    #[tokio::test]
    async fn test_execute_copy_overwrites_conflicts_when_requested() {
        let tmp = tempfile::TempDir::new().unwrap();
        let pool = setup_test_db().await;
        ...
        let plan = build_copy_plan(&pool, &source.to_string_lossy(), &target.to_string_lossy())
            .await
            .unwrap();
        let result = execute_copy_plan_impl(&plan, true).unwrap();

        assert_eq!(result.copied, 1);
        assert_eq!(result.overwritten, 1);
        assert_eq!(result.skills[0].overwritten, true);
        // Existing content replaced by the source version.
        assert_eq!(
            std::fs::read_to_string(target.join(".claude/skills/skill-b/SKILL.md")).unwrap(),
            "---\nname: b\n---\n"
        );
    ...
    #[tokio::test]
    async fn test_execute_copy_records_copied_skills_in_discover_cache() {
        let tmp = tempfile::TempDir::new().unwrap();
        let pool = setup_test_db().await;
        ...
        let plan = build_copy_plan(&pool, &source.to_string_lossy(), &target.to_string_lossy())
            ...
            .unwrap();
        let result = execute_copy_plan_impl(&plan, false).unwrap();
        record_copied_skills_in_discover_cache(&pool, &plan, &result)
            .await
            .unwrap();
        ...
        let target_row = rows
            .iter()
            .find(|row| row.project_path == target.to_string_lossy())
            .expect("copied skill should be registered under the target project");
        assert_eq!(target_row.dir_path, target.join(".claude/skills/skill-a").to_string_lossy());
        assert_eq!(target_row.platform_id, "claude-code");
        assert_eq!(target_row.id, "claude-code__proj-b__skill-a");
    }

    // ── Collection ↔ project integration ───────────────────────────────────

    /// Create a central skill dir + DB row + collection membership. Returns the
    /// created skill dir path.
    async fn create_central_collection_skill(
        ...
        let dir = tmp.path().join("central").join(skill_dir);
        std::fs::create_dir_all(&dir).unwrap();
        std::fs::write(
            dir.join("SKILL.md"),
            format!("---\nname: {}\n---\n", marker),
        ...
        db::upsert_skill(
            pool,
            &db::Skill {
                id: skill_dir.to_string(),
                name: skill_dir.to_string(),
                description: None,
        ...
        assert!(!collection_id.is_empty(), "a collection id is required");
        sqlx::query(
    ...
    #[tokio::test]
    async fn test_project_platform_dirs_detects_skill_subdirs() {
        let tmp = tempfile::TempDir::new().unwrap();
        let pool = setup_test_db().await;

        let project = tmp.path().join("proj-a");
        std::fs::create_dir_all(&project).unwrap();
        create_copy_source_skill(&project, ".claude/skills/skill-a", "a");
        create_copy_source_skill(&project, ".cursor/skills/skill-b", "b");

        seed_discovered_for_copy(
            &pool,
            "claude-code__proj-a__skill-a",
            "skill-a",
            &project.join(".claude/skills/skill-a").to_string_lossy(),
        ...
        seed_discovered_for_copy(
            &pool,
            "cursor__proj-a__skill-b",
            "skill-b",
            &project.join(".cursor/skills/skill-b").to_string_lossy(),
            &project.to_string_lossy(),
            "proj-a",
            "cursor",
        )
        ...
        let dirs = project_platform_dirs_impl(&pool, &project.to_string_lossy())
            .await
            .unwrap();
        assert_eq!(dirs, vec![".claude/skills", ".cursor/skills"]);
    }

    #[tokio::test]
    async fn test_collection_install_plan_previews_conflicts() {
        let tmp = tempfile::TempDir::new().unwrap();
        let pool = setup_test_db().await;

        let project = tmp.path().join("proj-b");
        std::fs::create_dir_all(&project).unwrap();
        create_copy_source_skill(&project, ".claude/skills/skill-b", "target-b");

        // Two central tools in the collection; one conflicts with the target.
        let collection = db::create_collection(&pool, "Test Collection", None)
            .await
            .unwrap();
        let _skill_a =
            create_central_collection_skill(&tmp, &pool, &collection.id, "skill-a", "a").await;
        let _skill_b =
            create_central_collection_skill(&tmp, &pool, &collection.id, "skill-b", "b").await;

        let plan = build_collection_install_plan(
            &pool,
            &collection.id,
            &project.to_string_lossy(),
            ".claude/skills",
        ...
        assert_eq!(plan.collection_name, "Test Collection");
        assert_eq!(plan.total_count, 2);
        assert_eq!(plan.conflict_count, 1);
        assert_eq!(plan.missing_count, 0);
        assert_eq!(plan.platform_dir, ".claude/skills");

        let item_a = plan.skills.iter().find(|s| s.dir_name == "skill-a").unwrap();
        assert!(!item_a.conflict);
        assert_eq!(
            item_a.target_dir,
            project.join(".claude/skills/skill-a").to_string_lossy()
        );
        let item_b = plan.skills.iter().find(|s| s.dir_name == "skill-b").unwrap();
        assert!(item_b.conflict);
    }

    #[tokio::test]
    async fn test_collection_install_rejects_missing_project() {
        let tmp = tempfile::TempDir::new().unwrap();
        let pool = setup_test_db().await;
        let collection_id = db::create_collection(&pool, "Col", None)
            .await
            .unwrap()
            .id;

        let missing = tmp.path().join("nope");
        let err = build_collection_install_plan(
            &pool,
            &collection_id,
    ...
    #[tokio::test]
    async fn test_collection_install_skips_conflicts_without_overwrite() {
        let tmp = tempfile::TempDir::new().unwrap();
        let pool = setup_test_db().await;
        ...
        .await
        .unwrap();
        let result = execute_collection_install_impl(&plan, false).unwrap();

        assert_eq!(result.copied, 1);
        assert_eq!(result.skipped, 1);
        assert_eq!(result.overwritten, 0);
        assert!(project.join(".claude/skills/skill-a/SKILL.md").exists());
        // Conflicting skill untouched.
        assert_eq!(
            std::fs::read_to_string(project.join(".claude/skills/skill-b/SKILL.md")).unwrap(),
            "---\nname: target-b\n---\n"
        );
    }

    #[tokio::test]
    async fn test_collection_install_overwrites_conflicts_when_requested() {
        let tmp = tempfile::TempDir::new().unwrap();
        let pool = setup_test_db().await;
        ...
        .await
        .unwrap();
        let result = execute_collection_install_impl(&plan, true).unwrap();
    ...
    #[tokio::test]
    async fn test_collection_install_records_cache_rows() {
        let tmp = tempfile::TempDir::new().unwrap();
        let pool = setup_test_db().await;
        ...
        .unwrap();
        let result = execute_collection_install_impl(&plan, false).unwrap();
        record_collection_install_in_discover_cache(&pool, &plan, &result)
            .await
            .unwrap();
        ...
        let row = rows
            .iter()
            .find(|row| row.project_path == project.to_string_lossy())
            .expect("installed skill should be registered under the target project");
        assert_eq!(row.dir_path, project.join(".claude/skills/skill-a").to_string_lossy());
        assert_eq!(row.id, "claude-code__proj-b__skill-a");
    }

    #[tokio::test]
    async fn test_add_discovered_skill_plan_reports_conflicts() {
        let tmp = tempfile::TempDir::new().unwrap();
        let pool = setup_test_db().await;
        let central_dir = tmp.path().join("central");
        std::fs::create_dir_all(&central_dir).unwrap();

        // Existing central skill occupying the name.
        let dir = central_dir.join("deploy");
        std::fs::create_dir_all(&dir).unwrap();
        ...
        .await
        .unwrap();
        let collection = db::create_collection(&pool, "Conflict Col", None)
            .await
            .unwrap();

        // Project skill being added.
        let project = tmp.path().join("proj-a");
        std::fs::create_dir_all(&project).unwrap();
        let skill_dir = create_copy_source_skill(&project, ".claude/skills/deploy", "project");
        seed_discovered_for_copy(
            &pool,
            "claude-code__proj-a__deploy",
            "deploy",
            &skill_dir.to_string_lossy(),
            &project.to_string_lossy(),
            "proj-a",
        ...
        let plan = build_add_discovered_skill_plan(
            &pool,
            &collection.id,
            "claude-code__proj-a__deploy",
            &central_dir,
        ...
        .await
        .unwrap();
        assert_eq!(plan.skill_dir_name, "deploy");
        assert!(!plan.central_conflict, "central conflict is no longer flagged");
    }

    #[tokio::test]
    async fn test_add_discovered_skill_links_collection_without_copying_to_central() {
        let tmp = tempfile::TempDir::new().unwrap();
        let pool = setup_test_db().await;
        let central_dir = tmp.path().join("central");
        std::fs::create_dir_all(&central_dir).unwrap();

        let collection = db::create_collection(&pool, "Project Col", None)
            .await
            .unwrap();

        let project = tmp.path().join("proj-a");
        std::fs::create_dir_all(&project).unwrap();
        let skill_dir = create_copy_source_skill(&project, ".claude/skills/my-project-skill", "project-skill");
        seed_discovered_for_copy(
            &pool,
            "claude-code__proj-a__my-project-skill",
            "my-project-skill",
            &skill_dir.to_string_lossy(),
            &project.to_string_lossy(),
            "proj-a",
        ...
        let result = execute_add_discovered_skill_to_collection_impl(
            &pool,
            &collection.id,
            "claude-code__proj-a__my-project-skill",
            false,
        ...
        .await
        .unwrap();
        assert_eq!(result.skill_id, "my-project-skill");
        assert!(result.added_to_collection);
        assert!(!result.overwrote_central);

        // Crucial: Central copy must NOT be created!
        let central_target = central_dir.join("my-project-skill");
        assert!(!central_target.exists(), "skill should not be copied to central directory");

        // Collection contains the skill.
        let skills = db::get_collection_skills(&pool, &collection.id).await.unwrap();
        assert_eq!(skills.len(), 1);
        assert_eq!(skills[0].id, "my-project-skill");
        assert!(!skills[0].is_central, "skill should not be marked as central");
        assert_eq!(skills[0].canonical_path.as_deref(), Some(skill_dir.to_string_lossy().as_ref()));
    }

    #[tokio::test]
    async fn test_batch_add_discovered_skills_to_collections() {
        let tmp = tempfile::TempDir::new().unwrap();
        let pool = setup_test_db().await;
        let central_dir = tmp.path().join("central");
        std::fs::create_dir_all(&central_dir).unwrap();

        let col_a = db::create_collection(&pool, "Col A", None).await.unwrap();
        let col_b = db::create_collection(&pool, "Col B", None).await.unwrap();

        let project = tmp.path().join("proj-a");
        std::fs::create_dir_all(&project).unwrap();
        let s1_dir = create_copy_source_skill(&project, ".claude/skills/skill-1", "s1");
        let s2_dir = create_copy_source_skill(&project, ".claude/skills/skill-2", "s2");

        seed_discovered_for_copy(
            &pool,
            "claude-code__proj-a__skill-1",
            "skill-1",
            &s1_dir.to_string_lossy(),
            &project.to_string_lossy(),
            "proj-a",
            "claude-code",
        )
        ...
        seed_discovered_for_copy(
            &pool,
            "claude-code__proj-a__skill-2",
            "skill-2",
            &s2_dir.to_string_lossy(),
            &project.to_string_lossy(),
            "proj-a",
            "claude-code",
        )
        ...
        batch_add_discovered_skills_to_collections_impl(
            &pool,
            &[col_a.id.clone(), col_b.id.clone()],
            &[
                "claude-code__proj-a__skill-1".to_string(),
                "claude-code__proj-a__skill-2".to_string(),
            ],
            &central_dir,
        )
        .await
        ...
        // No files in central
        assert!(!central_dir.join("skill-1").exists());
        assert!(!central_dir.join("skill-2").exists());

        // Col A has both skills
        let skills_a = db::get_collection_skills(&pool, &col_a.id).await.unwrap();
        assert_eq!(skills_a.len(), 2);
        let ids_a: Vec<&str> = skills_a.iter().map(|s| s.id.as_str()).collect();
        assert!(ids_a.contains(&"skill-1"));
        assert!(ids_a.contains(&"skill-2"));

        // Col B has both skills
        let skills_b = db::get_collection_skills(&pool, &col_b.id).await.unwrap();
        assert_eq!(skills_b.len(), 2);
    }

    #[tokio::test]
    async fn test_permanent_delete_removes_dir_and_row() {
        let tmp = tempfile::TempDir::new().unwrap();
        let pool = setup_test_db().await;

        let project = tmp.path().join("proj-a");
        std::fs::create_dir_all(&project).unwrap();
        let skill_dir = create_copy_source_skill(&project, ".claude/skills/doomed", "doomed");
        let dir_path = skill_dir.to_string_lossy().into_owned();
        seed_discovered_for_copy(
            &pool,
            "claude-code__proj-a__doomed",
            "doomed",
            &dir_path,
            &project.to_string_lossy(),
            "proj-a",
        ...
        let result = delete_discovered_skill_permanently_impl(&pool, "claude-code__proj-a__doomed")
            .await
            .unwrap();
        assert_eq!(result.removed_dir, dir_path);
        assert!(result.row_deleted);

        // Dir gone from disk and row gone from cache.
        assert!(!Path::new(&dir_path).exists());
        assert!(
            db::get_discovered_skill_by_id(&pool, "claude-code__proj-a__doomed")
                .await
                .unwrap()
                .is_none()
        );
    ...
    #[tokio::test]
    async fn test_permanent_delete_missing_dir_still_drops_row() {
        let tmp = tempfile::TempDir::new().unwrap();
        let pool = setup_test_db().await;

        let project = tmp.path().join("proj-a");
        std::fs::create_dir_all(&project).unwrap();
        // Register a row whose dir has already vanished from disk.
        let dir_path = project.join(".claude/skills/ghost").to_string_lossy().into_owned();
        seed_discovered_for_copy(
            &pool,
            "claude-code__proj-a__ghost",
            "ghost",
            &dir_path,
            &project.to_string_lossy(),
            "proj-a",
        ...
        let result = delete_discovered_skill_permanently_impl(&pool, "claude-code__proj-a__ghost")
            .await
            .unwrap();
        assert!(result.row_deleted);
        assert!(
            db::get_discovered_skill_by_id(&pool, "claude-code__proj-a__ghost")
                .await
                .unwrap()
                .is_none()
        );
